...
=======
// api for spoonacular that grabs recipies based off ingredients list
fetch('https://api.spoonacular.com/recipes/findByIngredients?apiKey=c469449ee5f84008b0b035ee3d1929a8&ingredients=apples,+flour,+sugar&number=2')
...
// 		"X-RapidAPI-Key": "12c3e2ac53fc4db0adfb72e431c95e92"
...
function renderIngredientsList(){
    ...
    ingredientsList.forEach((value, key) => {
        ...
        $(`<li id="listbox-item-${index}" role="option" class="text-gray-900 cursor-default hover:bg-indigo-500 hover:text-white select-none relative py-2 pl-3 pr-9">
            <div class="flex items-center">
                <span class="ml-3 block font-normal truncate">${userInput}</span>
            </div>
            <span class="absolute inset-y-0 right-0 flex items-center pr-4">
            <button type="button" class="delete h-6 w-6 px-2 bg-pink-600 hover:bg-pink-700 focus:ring-pink-500 focus:ring-offset-pink-200 text-white w-full transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2  rounded-full">
                Delete
              </button>
            </span>
        </li>`);
        ...
        $(".delete").on("click", function(){
            ...
            var liObject = buttonObject.parent().parent();
            var userInput = liObject.find("div span").text();
            ingredientsList = ingredientsList.filter(function (value){
